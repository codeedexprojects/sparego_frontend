...
const ProductForm = ({ productId, onSuccess, onCancel }) => {
  ...
  // REMOVED: All section-based filtering useEffect hooks
  ...
  useEffect(() => {
    if (formData.mainCategory && categories?.length > 0) {
      ...
      setFormData(prev => ({
        ...prev,
        category: filtered.some(cat => cat._id === prev.category) ? prev.category : '',
        subCategory: '',
        subSubCategory: ''
      }));
    }
  ...
  useEffect(() => {
    if (formData.category && subCategories?.length > 0) {
      ...
      setFormData(prev => ({
        ...prev,
        subCategory: filtered.some(cat => cat._id === prev.subCategory) ? prev.subCategory : '',
        subSubCategory: ''
      }));
    }
  ...
  useEffect(() => {
    if (formData.subCategory && subSubCategories?.length > 0) {
      ...
      setFormData(prev => ({
        ...prev,
        subSubCategory: filtered.some(cat => cat._id === prev.subSubCategory) ? prev.subSubCategory : ''
      }));
    }
