...
const SearchResultsPage = () => {
  ...
  useEffect(() => {
    if (query && (!searchResults || searchResults.length === 0)) {
      dispatch(searchProducts({ section: "SpareParts", search: query }));
    }
  }, [query, dispatch]);
  ...
  useEffect(() => {
    if (wishlist && wishlist.length > 0) {
      const wishlistIds = new Set(
        wishlist.map((item) => item._id || item.productId)
      );
  ...
  return (
    <div className="mx-auto p-6 bg-gray-50 min-h-screen">
      <h2 className="text-xl text-black font-semibold mb-6">Search Results</h2>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50 flex items-center"
                >
