...
export default function PromotionalBannerSection() {
  ...
  const { subCategoryBanners, loading, error } = useSelector((state) => state.banner);

  useEffect(() => {
    dispatch(getsubCategoryBanners());
  }, [dispatch]);

  
  const limitedBanners = subCategoryBanners?.banners?.slice(0, 2) || [];
