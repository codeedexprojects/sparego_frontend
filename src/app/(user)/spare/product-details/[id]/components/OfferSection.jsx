...
export default function PromotionalBannerSection() {
  ...
  const { productDetailBanners, loading, error } = useSelector((state) => state.banner);

  useEffect(() => {
    dispatch(getProductDetailBanners());
  }, [dispatch]);

  
  const limitedBanners = productDetailBanners?.banners?.slice(0, 2) || [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          {limitedBanners.map((promo, index) => {
            ...
            const productUrl = productId ? `/spare/product-details/${productId}` : `/section/category1`;
return(
            <div
              key={promo._id}
              className={`${
