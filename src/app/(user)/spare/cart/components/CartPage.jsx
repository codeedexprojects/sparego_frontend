...
const MyCart = () => {
  ...
  try {
    // FIXED: Now passing both productId and quantity
    ...
    if (updateCartItem.fulfilled.match(resultAction)) {
      ...
      toast.error(typeof errorMsg === 'string' ? errorMsg : "Failed to update cart");
    }
  } catch (error) {
 ...
 const handleRemoveItem = async (productId) => {
  ...
  try {
    // FIXED: Now passing just the productId string instead of an object
    const resultAction = await dispatch(removeFromCart(productId)); // Direct string, not object
    ...
    if (removeFromCart.fulfilled.match(resultAction)) {
      ...
      toast.error(typeof errorMsg === 'string' ? errorMsg : "Failed to remove item");
    }
  } catch (error) {
  ...
  return (
    <div className="mx-auto p-6 bg-gray-50 min-h-screen">
      ...
      <div className="space-y-6">
        {cart.items.map((item) => (
          <div key={item.product._id} className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm relative">
            {/* Remove Item Button */}
            <button
                ...
                <div className="mb-4">
                  ...
                  {item.discount > 0 && (
                    <span className="text-green-600 font-medium">
                      You save ₹{calculateSavings(item).toLocaleString()} ({item.discount}% off)
                    </span>
                  )}
                </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm mt-8">
        ...
        <div className="space-y-3 mb-6">
          <div className="flex justify-between">
            ...
            <span className="text-gray-900">₹{cart.cartTotal?.toLocaleString() || '0'}</span>
          </div>
          
          <div className="flex justify-between">
            <span className="text-gray-600">Discount</span>
            <span className="text-green-600">-₹{cart.cartDiscount?.toLocaleString() || '0'}</span>
          </div>
