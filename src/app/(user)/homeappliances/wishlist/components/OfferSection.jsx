...
import { getdynamicWishlistBanners } from "@/redux/slices/dynamicBannerSlice";
...
export default function PromotionalBannerSection({ page = "wishlist" }) {
  ...
  useEffect(() => {
    ...
    if (id) {
      setSectionId(id); 
      dispatch(getdynamicWishlistBanners(id)); 
    }
  }, [dispatch]);
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
          {limitedBanners.map((promo, index) => {
           ...
           const productUrl = productId ? `/homeappliances/product-details/${productId}` : `/homeappliances/wishlist`;
return (
            <div
              key={promo._id}
              className={`${
