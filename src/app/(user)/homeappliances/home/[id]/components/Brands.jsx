...
<section className="py-12 px-4 sm:px-6 lg:px-16 bg-white"> 
  ...
  <div className="overflow-x-auto sm:overflow-x-visible -mx-4 px-4 sm:mx-0 sm:px-0">
    <div className="flex gap-4 sm:grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 sm:justify-items-center min-w-max sm:min-w-0"> 
      {brands?.map((brand) => ( 
        <div 
          key={brand._id} 
          className="flex flex-col items-center group cursor-pointer flex-shrink-0" 
        > 
          <div className="w-28 h-28 flex items-center justify-center border border-gray-400 rounded-lg bg-white shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden"> 
            {/* Hover red background effect */} 
